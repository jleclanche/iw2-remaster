...
|---|---|---|---|---|
| **0 — Prologue** | iprelude, m10, m20, m40, m50, m60, generaltraining, missiontour | m20: the remote-link weapons range is an OPTIONAL lesson, settled from source — the mission's one objective `a0_m20_objectives_complete_course` (added iact0mission20.pog:1023) completes on course time alone (:315, Storm Petrel + `g_act0_proving_grounds_complete`), the range (`local_15514`) never touches it, and its hit loop credits the player's own ship when no remote is linked (:842) | m20: `WeaponsUseExplicitTarget`, `SetCollision` staging; m50: `BrightnessOf` branch reads 0 | m10 VERIFIED to first waypoint + checkpoint (campcheck); rest of m10's HUD-tour and m40/m50/m60 never driven end-to-end; icPopUpCommsScreen overlay unverified |
