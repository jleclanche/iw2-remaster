...
def native_returns() -> dict[str, list[str]]:
    """fqn (lower) -> the return expressions of its GDScript implementation.

    Godot coerces int/float/bool silently at a typed return, but null raises
    a script error. So before a facade return is typed, the implementation's
    returns are checked: a native that can hand back null stays Variant."""
    ...
    for f in NATIVES.glob("*.gd"):
        ...
        for line in f.read_text(encoding="utf-8",
            ...
            mk = _MARK.search(line)
            if mk and line.lstrip().startswith("#"):
                if mk.group(1) != "pkg":
                    pending.append("%s.%s" % (mk.group(1),
                                              mk.group(2).lower()))
                continue
            ...
            if line and line[0] not in " \t#":
                pending, current = [], []    # some other top-level construct
                continue
            r = ret_re.match(line)
            if r and current:
                for fqn in current:
                    out[fqn].append(r.group(1))
    return out


def _null_capable(rets: list[str] | None) -> bool:
    """Can this implementation's return reach the caller as null?"""
    if not rets:
        return True     # not found, or no returns at all: falls off as null
    for e in rets:
        if not e:
            return True                    # bare `return` is null
        if re.match(r'^(null\b|"|\[|\{)', e):
            return True
        if e.startswith("null if") or re.search(r"else\s+null$", e):
            return True
    return False


def _facade_type(declared: str, verdict: str | None,
                 rets: list[str] | None) -> str:
    """The validated GDScript return type for a facade method, or ''.

    Typed only where pogret's handler audit (#24) confirms the SDK
    declaration: the binary demonstrably writes the slot with that type.
    POG's bool is a 32-bit int and ported callers do arithmetic on it
    (`1 - task.IsRunning(t)`), so declared-bool natives type as int.
    Handles/strings/lists stay Variant: our runtime's representations
    differ from raw 32-bit words."""
    if verdict is None or _null_capable(rets):
        return ""
    if declared == "float" and verdict == "float":
        return "float"
    if declared in ("int", "bool") and verdict in ("int", "bool"):
        return "int"
    return ""


def gen_native_api() -> tuple[str, int, int]:
    ...
    # from. Numeric returns are typed where the handler audit validates the
    # declaration (_facade_type); everything else stays Variant.
    try:
        sigs = signatures()
    except SystemExit:
        sigs = {}                      # SDK not installed: names stay a0, a1
    try:
        from .pogret import audit
        verdicts: dict[str, str] = audit()
    except ImportError:
        verdicts = {}                  # capstone not installed: stay Variant
    rets = native_returns()

    n_typed = n_typeable = 0
    for pkg, fns in pkgs.items():
        ...
        for fn in fns:
            ...
            key = "%s.%s" % (pkg.lower(), fn.lower())
            ret, params = sigs.get(key, ("", []))
            ...
            if params or ret:
                ...
                L.append("\t## prototype %s %s.%s(%s)"
                         % (ret or "void", pkg.capitalize(), fn,
                            " %s " % decl if decl else ""))
            t = _facade_type(ret, verdicts.get(key), rets.get(key))
            if ret in ("int", "bool", "float") \
                    and verdicts.get(key) in ("int", "bool", "float"):
                n_typeable += 1
            if t:
                n_typed += 1
            L.append("\tfunc %s(%s) -> %s:" % (_fname(fn), args,
                                               t or "Variant"))
...
def main() -> None:
    ...
    api, n_typed, n_typeable = gen_native_api()
    (GEN / "native_api.gd").write_text(api, encoding="utf-8")
    print("native facade: %d numeric returns typed "
          "(%d validated; the rest have null-capable implementations)"
          % (n_typed, n_typeable))
