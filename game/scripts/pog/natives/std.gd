...
func _load_csv(path: String) -> void:
	...
	if f == null:
		return
	# The extraction normalises the tables to strict CSV (tools/iw2/pogdata.py
	# strips the quotes out of ';' comment lines and repairs the four shipped
	# unbalanced-quote typos), so the plain reader is safe here.
