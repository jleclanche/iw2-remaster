...
func local_12188(v0, v1) -> Variant:
	...
	if _pog_is_null(v2):
		...
		while true:
			await _pog_wait(1.0)
			if not (sim.distance_between(v5, v3) > 50000.0):
				break
		if PogRuntime.TRACE:
	...
	while true:
		await _pog_wait(1.0)
		if not (sim.distance_between(v5, v2) > 5000.0):
			break
	if PogRuntime.TRACE:
...
func setup_trade_delivery(v0) -> Variant:
	...
	v5 = 0
	while v5 < object.int_property(v0, "cargo_reward_double_counter"):
...
func resolve_delivery(v0) -> Variant:
	...
	if _pog_is_null(object.bool_property(v0, "personal_delivery")):
		...
		while true:
			...
			if not (sim.distance_between(v4, v5) > 50000.0):
				break
