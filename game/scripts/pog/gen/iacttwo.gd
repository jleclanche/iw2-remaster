...
func local_14092() -> Variant:
	...
	while true:
		...
		if not (v6) and not (v9):
			...
			while true:
				await _pog_wait(1.0)
				if not (sim.distance_between(iship.cast(group.leader(v0)), v5) < 350000.0):
					break
			group.destroy(v0, 1)
			return
	return
	...
	match state.progress(v1):
		0:
			while true:
				...
				await _pog_wait(2.0)
				if not (sim.distance_between(v3, v4) > 7000.0):
					break
			await iconversation.begin()
		...
		match state.progress(v1):
			...
			2:
				while true:
					...
					await _pog_wait(1.0)
					if not (sim.distance_between(v4, v13) > 8000.0):
						break
				await iconversation.begin()
						...
						if sim.distance_between(v3, v2) < 10000.0:
							if global.pog_bool("g_act2_completed_blockade_run") == 1 or global.exists("g_blockade_runner_running"):
								...
								if v4:
									...
									v4 = 1
									match global.pog_int("g_act2_mwari_conversations"):
										0:
											await iconversation.begin()
		...
		1:
			...
			if object.int_property(v2, "delay") < 240:
				while true:
					...
					if not (object.int_property(v2, "delay") < 120):
						break
...
func hoffer_creation_handler() -> Variant:
	...
	while true:
		...
		v1 = global.string("g_player_base_system")
		v2 = imapentity.find_by_name_in_system("Lucrecia's Base", v1)
		if not _pog_eq(isim.active_world(), v1):
			continue
		if not (global.pog_bool("g_act3_ready_for_mission_ten") != 1 and _pog_is_null(global.exists("g_hoffer_disabled"))):
			continue
		if sim.distance_between(v4, v3) > 300000.0 and not (v5):
			...
			if not (v5 and sim.distance_between(v4, v3) < 300000.0):
				continue
			...
			17:
				...
				if _pog_is_null(global.pog_bool("g_skip_to_specific")):
					...
					while true:
						...
						await _pog_wait(1.0)
						if not (object.int_property(v1, "a2_m24_waiter") < 120):
							break
...
func local_68543(v0, v1) -> Variant:
	...
	while true:
		...
		await _pog_wait(0.10000000149011612)
		if not (sim.distance_between_centres(v4, v1) > 200.0):
			break
