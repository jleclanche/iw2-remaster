...
func local_2479(v0, v1, v2) -> Variant:
	...
	while true:
		if _pc == 2479:
			...
			if _pog_is_null(group.sim_count(v0)) and state.progress(v2) != 100:
				_pc = 2985
				continue
			else:
				_pc = 3411
...
func local_3426(v0, v1, v2) -> Variant:
	...
	while true:
		if _pc == 3426:
			...
			if sim.distance_between(v0, v1) > 3000.0 and not (v4):
				_pc = 3578
				continue
			else:
				_pc = 3633
...
func local_5431(v0) -> Variant:
	...
	while true:
		if _pc == 5431:
			...
			if not (await iutilities.player_in_range(v6)) and v1 == 1:
				_pc = 5921
				continue
			else:
				_pc = 5998
...
func mission_handler() -> Variant:
	...
	while true:
		if _pc == 6032:
			...
			if state.progress(v4) == 6 or state.progress(v4) == 100:
				_pc = 6662
				continue
			else:
				_pc = 6960
