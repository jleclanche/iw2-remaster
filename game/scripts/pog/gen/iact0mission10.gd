...
func local_1276(v0, v1, v2, v3) -> Variant:
	...
	while true:
		...
		if state.progress(v3) == 100:
			...
			group.destroy(v7, 1)
			return
		if not _pog_eq(iship.current_target(v0), isim.cast(v8)):
			continue
		break
	...
	while true:
		...
		if v4:
			if v5:
				...
				while true:
					await _pog_wait(1.0)
					if not (iai.current_order_type(v0) != 2):
						break
				ihud.set_prompt("", "")
			...
			while true:
				await _pog_wait(1.0)
				if not (iai.current_order_type(v0) != 3):
					break
			ihud.set_prompt("", "")
			v9 = iai.current_order_target(v0)
...
func local_3664(v0, v1, v2) -> Variant:
	...
	while true:
		await _pog_wait(0.5)
		if not (not _pog_eq(ihud.current_menu_node(), "hud_menu_map")):
			break
	ihud.set_prompt("", "")
	...
	while true:
		await _pog_wait(0.5)
		if not (not _pog_eq(object.string_property(iship.current_target(v0), "name"), object.string_property(v1, "name"))):
			break
	ihud.set_prompt("", "")
...
func mission_handler() -> Variant:
	...
	while true:
		...
		if not (state.progress(v9) == 8 or state.progress(v9) == 100):
			continue
