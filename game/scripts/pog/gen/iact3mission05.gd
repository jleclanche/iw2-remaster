...
func local_1778(v0, v1, v2) -> Variant:
	...
	while true:
		...
		if not (state.progress(v2) == 2 and not (await iutilities.player_in_range(v1))):
			continue
...
func local_2445(v0, v1, v2, v3) -> Variant:
	...
	while true:
		...
		if not (v4 and not (v5)):
			continue
		if not (iai.is_order_complete(v7) or sim.distance_between(v7, v0) < 700.0):
			continue
		v5 = 1
	...
	while true:
		...
		if not _pog_eq(isim.active_world(), "map:/geog/gagarin/owens_star"):
			continue
		if not (isim.is_docked_to_structure(isim.cast(v0), isim.cast(v1)) and not (v6)):
			continue
		v6 = 1
